...
* `POSTGRES_USER` and `POSTGRES_PASSWORD`: Sets the root credentials for the PostgreSQL database instance.
...
* `PGADMIN_DEFAULT_EMAIL` and `PGADMIN_DEFAULT_PASSWORD`: Sets the initial login credentials for the pgAdmin web interface.
...
Once you have completed the first-time setup, you no longer need to build the images from scratch. You can manage the pipeline using these standard commands:

To start the environment normally:
```bash
docker compose up -d
```

To shut down the environment (Preserves your data):
```bash
docker compose down
```

To completely wipe the environment (Deletes containers and data volumes):
If you want to start completely fresh and delete all ingested data, Kestra history and pgAdmin settings, run:
```bash
